...
const PTV = {
    ...
    run: function (params: StartParams) {
        ...
        let validateDeparturesResponse = function (
            departuresResponse: V3DeparturesResponse
            ...
            return new Promise<V3DeparturesResponse>((resolve, reject) => {
                ...
                if (departuresResponse.runs == undefined) {
                    reject(
                        "Departures error: Departure response contained no runs."
                    );
                }
        ...
        let validateStopsOnRouteResponse = function (
            resolvedPromises: [
                StopsOnRouteCache,
                V3StoppingPatternResponse,
        ...
        let validateStoppingPatternResponse = function (
            resolvedPromises: [
                StopsOnRouteCache,
                V3StoppingPatternResponse,
                V3DisruptionsResponse
        ...
        let validateDisruptionsResponse = function (
            resolvedPromises: [
                StopsOnRouteCache,
                V3StoppingPatternResponse,
                V3DisruptionsResponse
        ...
        let updateStopsOnRouteCache = function (
            stopsOnRouteResponse: V3StopsOnRouteResponse | undefined,
            cache: StopsOnRouteCache,
            cacheKey: string
        ...
        Promise.all([departuresPromise, prepareStopsOnRouteCachePromise])
            .then((resolvedPromises) => {
                const departuresResponse: V3DeparturesResponse =
                    resolvedPromises[0];
                ...
                let stopsOnRoutePromise = _PTV_v2.requestStopsOnRoute(
                    ...
                    .then((stopsOnRouteResponse) =>
                        updateStopsOnRouteCache(
                            stopsOnRouteResponse,
                            resolvedPromises[1],
                            cacheKey
                        )
                    )
                ...
                Promise.all([
                    ...
                    .then((resolvedPromises) => {
                        const stopsOnRoute =
                            resolvedPromises[0].data!.get(cacheKey);
...
*/
    prepareStopsOnRouteCache: function (
        existingCache: StopsOnRouteCache
    ): Promise<StopsOnRouteCache> {
        return new Promise(function (resolve) {
            ...
            console.log(
                "Empty cache created with date",
                new Date(expiryDateNew).toDateString()
            );
            //Try and load the cache from local storage
            if (BrowserHelpers.hasLocalStorage()) {
                const jsonFromStorage = localStorage.getItem(
                    PTV.localStorageCacheKey
                );
                ...
                if (cacheFromStorage != undefined) {
                    ...
                    if (
                        new Date(cacheFromStorage.date).getTime() <=
                        new Date().getTime()
                    ) {
                        console.log(
                            "Removing cache due to expired date",
                            new Date(cacheFromStorage.date).toDateString()
                        );
    ...
    getDisruptionDataForDeparture: function (
        departure: V3Departure,
        disruptionsMap: Map<number, V3Disruption>
    ): DisruptionsForDeparture {
        ...
        for (let i = 0; i < departure.disruption_ids.length; i++) {
            ...
            const disruptionType =
                data.disruption_type != undefined
    ...
    updatePage: function (
        ...
        return new Promise(function (resolve, reject) {
            ...
            const disruptionsMap = PTV.buildDisruptionsMap(
                metroTrainDisruptions
            );
            ...
            document.body.setAttribute(
                "data-colour",
                PTV.getColourForRoute(nextDepartureRouteId)
            );
            ...
            if (runs == undefined)
                reject(
                    "No runs returned for run " + nextDeparture.run_id + "."
                );

            const nextDepartureName =
                runs![nextDeparture.run_id!].destination_name;
            ...
            if (nextDeparture.disruption_ids != null) {
                const disruption_data = PTV.getDisruptionDataForDeparture(
                    nextDeparture,
                    disruptionsMap
                );
                const disruptionElement = document.getElementById(
                    "next-dest-disruption"
                )!;
                disruptionElement.setAttribute(
                    "class",
                    "clearable " + disruption_data.className
                );

                const disruption_list = document.getElementById(
                    _disruptionListElementId
                )!;

                const template =
                    "<small><strong>{type}: </strong>{message}</small>";
                const general_disruptions =
                    PTV.getGeneralDisruptions(disruptionsResponse);
                ...
                for (const d of disruption_data.items) {
                    if (d.type == undefined || d.message == undefined) continue;
            ...
            if (nextDeparture.scheduled_departure_utc == undefined)
                reject(
                    "No scheduled_departure_utc returned for next departure"
                );
            const time = DateTimeHelpers.formatSingleTime(
                nextDeparture.scheduled_departure_utc!,
                true
            );
            document.getElementById("next-time")!.innerHTML = time;

            //TODO immutable for diff
            let diff = DateTimeHelpers.getDifferenceFromNow(
                nextDeparture.estimated_departure_utc,
                nextDeparture.scheduled_departure_utc!
            ).toString();
            const diffSec = DateTimeHelpers.getDifferenceFromNowSec(
                nextDeparture.estimated_departure_utc,
                nextDeparture.scheduled_departure_utc!
            );
            ...
            for (let s = 0; s < stopsOnRoute.stops!.length; s++) {
                stops.set(
                    stopsOnRoute.stops![s].stop_id,
                    stopsOnRoute.stops![s].stop_name
                );
            }
            ...
            for (let j = 0; j < stoppingPattern.departures!.length; j++) {
                const stoppingPatternStopId =
                    stoppingPattern.departures![j].stop_id!;
            ...
            const fullList = getStoppingPatternWithSkippedStations(
                stopsFromCurrent,
                departures![0].route_id!,
                inbound,
                stopId
            );
            const desc = getShortStoppingPatternDescription(
                fullList,
                inbound,
                stopId
            );
            ...
            const directionText = nextRun.vehicle_position?.direction
                ...
                : "";
            const runDesc =
                nextRun.vehicle_descriptor?.description &&
            ...
            if (stoppingPatternCount > 7) {
                ...
                listElement.className = listElement.className.replace(
                    "two-columns",
                    ""
                );
            }
...
function updateView(): void {
    ...
    const new_url =
        window.location.pathname + updateQueryVariable("p", platform_number);
...
function getShortStoppingPatternDescription(
    stoppingPatternWithSkippedStations: IdNameSkipped[],
    isInbound: boolean,
    ...
    const isStoppingAtAnyLoopStation =
        isStoppingAtAnyCityLoopStation(notSkippedIds);
    const isNotRunningViaLoop = isNotRunningViaCityLoop(notSkippedIds);

    const nextNonLoopStationName =
        notSkipped.length >= 2 ? notSkipped[1].name : "";
...
function addFollowingDeparture(
    disruptionsMap: Map<number, V3Disruption>,
    departure: V3Departure,
    destinationName: string
): void {
    const time =
        departure.scheduled_departure_utc != undefined
            ? DateTimeHelpers.formatSingleTime(
    ...
    const diff =
        departure.scheduled_departure_utc != undefined
            ? DateTimeHelpers.getDifferenceFromNow(
    ...
    const timeWrapper = createElementWithContent(
        "div",
        "col-6 col-md-3 col-lg-2 order-2 order-md-1 ml-4 ml-md-0",
        "h4",
        "font-weight-light",
        time
    );
    ...
    disruptionSpan.setAttribute(
        "class",
        disruption_data.className + " clearable"
    );

    //Destination
    const destWrapper = document.createElement("div");
    destWrapper.setAttribute(
        "class",
        "col-12 col-md-6 col-lg-8 order-1 order-md-2"
    );
    ...
    const diffWrapper = createElementWithContent(
        "div",
        "col-5 col-md-3 col-lg-2 order-3 order-md-3 text-right",
        "h4",
        "font-weight-light text-white bg-dark d-inline px-2",
        diff + " min"
    );
    ...
    if (disruption_data.items.length > 0) {
        ...
        disruptionDetailWrapper.setAttribute(
            "class",
...
function getStoppingPatternWithSkippedStations(
    stopList: IdName[],
    lineId: number,
    isInbound: boolean,
    currentStopId: number
    ...
    for (let i = 0; i < allStops.length; i++) {
        ...
        if (foundCurrentStop) allStopsReduced.push(allStops[i]);

        //Check if we've found the last stop in the stopping pattern for this run
        if (allStops[i].stop_id * 1 == stopList[stopList.length - 1].id) break;
    }
...
function addStoppingPatternItem(
    name: string,
    isSkipped: boolean,
...
function isCityLoopStation(stopId: number) {
    return (
        stopId == _PARLIAMENT_ID || //Parliament
...
function isNotRunningViaCityLoop(stopIds: number[]) {
    return (
        stopIds.indexOf(_PARLIAMENT_ID) == -1 &&
...
function reverseArray(input: any[]): any[] {
    if (!input || input == undefined || input.length <= 0) return input;
